   ...
   if( $show_cart_to_visitors == 'no' && !is_user_logged_in() ){
	...
	if ( ! function_exists( 'zn_woocommerce_post_thumbnail_html' ) ) {
		...
		function zn_woocommerce_post_thumbnail_html( $html, $pid, $tid, $size )
		{
			...
			$woo_prodLoop_image_size = zget_option( 'woo_cat_image_size', 'zn_woocommerce_options' );
			...
			if ( ! empty( $woo_prodLoop_image_size ) ) {
				$width  = ( !empty( $woo_prodLoop_image_size['width'] ) ) ? $woo_prodLoop_image_size['width'] : '';
				$height = ( !empty( $woo_prodLoop_image_size['height'] ) ) ? $woo_prodLoop_image_size['height'] : '';
			}
			...
			$image = vt_resize( $tid, '', $width, $height, $woo_cat_image_crop );
			$imageUrl = $image['url'];
			...
			if(!empty($imageUrl)){
				...
				$img = '<img class="kw-prodimage-img" '.$src.' width="' . $image['width'] . '" height="' . $image['height'] . '" alt="'.esc_attr( get_post_meta($tid, '_wp_attachment_image_alt', true) ).'" title="'.esc_attr( get_the_title($tid) ).'">';
			}
...
function zn_woocommerce_loop_add_to_cart_link( $link ){
	...
	if( $product_layout == 'style2' ) {
		...
		if( strpos($link, get_permalink() ) ){
			$link = str_replace( $add_to_cart_text, $more_icon, $link );
		}
		else {
			$link = str_replace( $add_to_cart_text, $addtocart_icon, $link );
		}
...
function znpb_woo_add_kallyas_template( $current_layout, $post, $post_id ){
	...
	if( is_product() ){
		...
		$shop_archive = ZNPB()->add_module_to_layout( 'TH_ProductContent', array( 'num_columns' => $archive_columns ) );
