...
if(! class_exists('WpkPageHelper')) {
	...
	class WpkPageHelper
	{
		...
		public static function zn_background_source( $args = array() )
		{
			...
			$parallax = $sourceType == 'image' && $args['enable_parallax'] == 'yes' && !ZNPB()->is_active_editor;
			$bg_container_class='';
			...
			if ( $sourceType ):
				...
				if ( $sourceType == 'image' ) {
					...
					if ( !empty( $background_image ) ) {
						if ( $parallax ) {
							$bg_source .= '<img class="kl-bg-source__bgimage ww cover-fit-img" src="' . $args['source_background_image']['image'] . '">';
						}
						else{
			...
			if ( $bg_source != '' || $bg_overlay != '' ) {
				...
				if($parallax){
					echo '<div class="kl-bg-source__parallaxWrapper zn-parallaxHack--'.$args['source_background_image']['position']['y'] .'">';
				}
		...
		public static function zn_klvideo( $args = array() )
		{
			...
			$output = '<div class="kl-video-container '.$args['wrapper_class'].'">';
				...
				$output .= '<div class="kl-video kl-video--valign kl-video--halign" data-setup=\''.json_encode($video_options).'\' '.WpkPageHelper::zn_schema_markup('video').'></div>';
			$output .= '</div>';
